          ...
          <div className="fixed inset-0 z-50 flex justify-end bg-black/30 backdrop-blur-sm">
            <div className="bg-white w-full max-w-[420px] h-full shadow-2xl flex flex-col animate-[fadeUp_0.3s_ease-out]">
              ...
              <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-6 space-y-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="block text-[13px] font-bold text-[var(--color-text-secondary)] mb-1">Category</label><select value={formData.category} onChange={e=>setFormData({...formData, category: e.target.value as any})} className="w-full h-11 px-3 border border-[var(--color-border)] rounded-[var(--radius-sm)] outline-none focus:border-[var(--color-primary)]"><option>Vegetables</option><option>Fruits</option><option>Dairy</option><option>Grains</option></select></div>
