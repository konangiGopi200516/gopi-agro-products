...
router.post(
  ...
  async (req: express.Request, res: express.Response) => {
    ...
    try {
        ...
        if (existingUser.passwordHash) {
          ...
          if (isValid) {
            if (!existingUser.verified) {
              await db.ref(`users/${existingUid}`).update({ verified: true });
            }
      ...
      res.json({ message: "Registration successful. Welcome to KisanMart!", user: safeUser, accessToken: tokens.accessToken, refreshToken: tokens.refreshToken });
    } catch (error: any) {
