...
async function main() {
    ...
    const dirs = fs_1.default.readdirSync(publicDir, { withFileTypes: true })
    ...
    for (const dir of dirs) {
        ...
        const categoryPath = path_1.default.join(publicDir, dir.name);
        const files = fs_1.default.readdirSync(categoryPath)
        ...
        for (const file of files) {
            ...
            let unit = "kg";
            if (slug.includes('dairy'))
                unit = productName.toLowerCase().includes('milk') ? "litre" : "pack";
            else if (slug.includes('leafy'))
                unit = "bunch";
            const productData = {
                name: productName,
